...
class Plant_Processor:
    ...
    def __init__(self):
        ...
        self.output_path = os.path.join(os.path.expanduser('~'), 'Desktop', 'project', 'deepLeafSegmentation', 'synth_data', 'partial_cloud_pickles_3class_midres')
    ...
    def save_meshes(self,i):
        ...
        for split_object in bpy.data.objects: # Check for given object names
            ...
            if not 'Camera' in split_object.name and not 'Light' in split_object.name and not 'Empty' in split_object.name:
                ...
                dae_save_location = os.path.join(self.save_path, 'split_by_organ_dae_3class_midres', dae_save_string)
                obj_save_location = os.path.join(self.save_path, 'split_by_organ_obj_3class_midres', obj_save_string)
