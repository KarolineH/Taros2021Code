...
file_path = os.path.join(os.path.expanduser('~'), 'Desktop', 'project', 'arbaro', 'automatic_plant_generation')
save_path = os.path.join(os.path.expanduser('~'), 'Desktop', 'project', 'catkin_ws', 'src', 'model_capture', 'models')
for i in range(1,2):
    j = 0
    ...
    try:
        ...
        for split_object in bpy.data.objects:
            ...
            if "tomato" in split_object.name:
                ...
                dae_save_location = os.path.join(save_path, 'split_tomatoes_dae', dae_save_string)
                obj_save_string = '%s_%s.obj' %(i,j)
                obj_save_location = os.path.join(save_path, 'split_tomatoes_obj', obj_save_string)
